...
QString DataAccessor::get_login_by_id(const unsigned user_id) { 
  SQLDAL::Friend client_friend = user_->GetFriend(user_id);
...
QVector<SQLDAL::Friend> DataAccessor::get_friends() { 
  QVector<SQLDAL::Friend> client_friend = user_->GetFriends();
...
unsigned DataAccessor::get_id_by_login(const QString user_login){
  SQLDAL::Friend client_friend = user_->GetFriend(user_login);
...
QVector<SQLDAL::Message> DataAccessor::get_messages(const QString user_login) {
  QVector<SQLDAL::Message> messages = message_->GetMessages(user_login);
  return messages;
}

QVector<SQLDAL::Message> DataAccessor::get_messages(unsigned user_id) {
  QVector<SQLDAL::Message> messages = message_->GetMessages(user_id);
  return messages;
}
