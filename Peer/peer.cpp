...
bool Peer::ConnectToPeer(unsigned id) {
  ...
  QPair<QString, int> ip_port =
      client_dal_.GetIPPort(id);  // marko - change to QPair<QString, quint16>
                                  // GetIPPort(const unsigned& user_id);
  connections_[id]->connectToHost(ip_port.first, ip_port.second);
  QString logMessage =
      receiver_ip_.toString() + " : " + QString::number(ip_port.second);
  emit SendLog("trying connect to: " + logMessage);
  if (connections_[id]->waitForConnected(5000)) {
    emit SendLog("connected to:" + logMessage);
    connections_[id]->StartConnectionTimer(
        30000);  // 30 sec until disconnecting
    connect(connections_[id], SIGNAL(CoonectionTimeout()), this,
            SLOT(DisconncetFromPeer()));
    connect(connections_[id], SIGNAL(readyRead()), connections_[id],
            SLOT(ReceiveRequests()));

    connect(connections_[id], SIGNAL(SendLog(QString)), this,
            SIGNAL(SendLog(QString)));
    connect(connections_[id], SIGNAL(SendMessageToUI(QString)), this,
            SIGNAL(SendMessageToUI(QString)));
    return true;
  } else {
...
void Peer::SendUpdateInfo() {
  ...
  for (const QString& ip_to_send : friends_ip) {
    update_sender_.writeDatagram(to_write, QHostAddress(ip_to_send),
                                 my_listen_port_);
  }
...
void Peer::UpdateFriendsInfo() {
  ...
  while (update_receiver_.hasPendingDatagrams()) {
    ...
    client_dal_.UpdateIPPort(updated_friend_info.id, peer_address.toString(),
                             updated_friend_info.port);

    emit SendLog("updated " + client_dal_.GetLoginById(updated_friend_info.id) +
                 "'s info");
  }
}
...
bool Peer::LogIn(QString login, QString password) {
  ...
  connect(server_connection_, SIGNAL(SendLog(QString)), this,
          SIGNAL(SendLog(QString)));
  LoginInfo info;
...
void Peer::SetSocket(QTcpSocket* socket) {
  ...
  unsigned id = client_dal_.GetIDByIPPort(connection->peerAddress().toString(),
                                          connection->peerPort());
  connections_[id] = connection;

  connect(
      connection, SIGNAL(readyRead()), connection,
      SLOT(ReceiveRequests()));  // try to read line to \n when recieving data
  connect(connection, SIGNAL(SendLog(QString)), this, SIGNAL(SendLog(QString)));
  connect(connection, SIGNAL(SendMessageToUI(QString)), this,
          SIGNAL(SendMessageToUI(QString)));
